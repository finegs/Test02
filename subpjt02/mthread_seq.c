...
void release(pthread_t* tids, pthread_cond_t* conds, int* arr);
...
{ 
    ...
    if(!cond) { perror("fail to malloc : pthread_cond_t[]"); release(tid, cond, arr); return 1; }

    tid = (pthread_t*)malloc(sizeof(pthread_t) * threads); 
    if(!tid) { perror("fail to malloc : pthread_t[]"); release(tid, cond, arr);return 1; }

    arr = (int*)malloc(sizeof(int) * threads); 
    if(!arr) { perror("fail to malloc :int[]");release(tid, cond, arr); return 1; }
